...
def train_loss(model, train_inputs, train_labels, train_params, mini_batch_size, criterion, device):
    ...
    train_inputs = train_inputs.view(train_inputs.size(0), num_channels, train_inputs.size(-2), train_inputs.size(-1))
    train_labels = train_labels.view(train_labels.size(0), num_channels, train_labels.size(-2), train_labels.size(-1))
    train_params = train_params.view(train_params.size(0), num_channels, train_params.size(-2), train_params.size(-1))
    ...
    for i in range(0, train_inputs.size(0), train_inputs.size(0)):
        ...
        train_output = model(train_input, train_param)

        # calculate test_loss
        train_loss += criterion(train_output, train_label).cpu().data.numpy()

        train_outputs[i:i+mini_batch_size, :, :, :] = train_output
        n += 1
...
def test_loss(model, test_inputs, test_labels, test_params, mini_batch_size, criterion, device):
    ...
    # prepare to size the size of the tensor
    test_inputs = test_inputs.view(test_inputs.size(0), num_channels, test_inputs.size(-2), test_inputs.size(-1))
    test_labels = test_labels.view(test_labels.size(0), num_channels, test_labels.size(-2), test_labels.size(-1))
    test_params = test_params.view(test_params.size(0), num_channels, test_params.size(-2), test_params.size(-1))
    ...
    for i in range(0, test_inputs.size(0), test_inputs.size(0)):
        ...
        test_output = model(test_input, test_param)

        # calculate test_loss
        test_loss += criterion(test_output, test_label).cpu().data.numpy()

        test_outputs[i:i+mini_batch_size, :, :, :] = test_output
        n += 1
